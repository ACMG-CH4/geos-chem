...
!
    ...
    ! Arrays of clusters for applying emissions perturbations
    REAL(fp)           :: CLUSTERS(State_Grid%NX,State_Grid%NY)

    ! Array of scale factors for emissions (from HEMCO)
    REAL(fp)           :: EMIS_SF(State_Grid%NX,State_Grid%NY)

    ! Array of scale factors for wetland emissions (from HEMCO)
    REAL(fp)           :: WETLAND_SF(State_Grid%NX,State_Grid%NY)
    ...
    IF ( Input_Opt%AnalyticalInv ) THEN
       ...
       ! Evaluate the cluster field from HEMCO
       CALL HCO_EvalFld( HcoState, 'CH4_CLUSTERS', CLUSTERS, RC)
    ...
    IF ( Input_Opt%UseEmisSF ) THEN
       ...
       CALL HCO_EvalFld( HcoState, 'EMIS_SF', EMIS_SF, RC)
       IF ( RC /= GC_SUCCESS ) THEN
    ...
    IF ( Input_Opt%UseWetlandSF ) THEN

       ! Evaluate wetlands scale factors from HEMCO
       CALL HCO_EvalFld( HcoState, 'WETLAND_SF', WETLAND_SF, RC)
       IF ( RC /= GC_SUCCESS ) THEN
          ErrMsg = 'WETLAND_SF not found in HEMCO data list!'
          CALL GC_Error( ErrMsg, RC, ThisLoc )
          RETURN
       ENDIF
    ...
    ! =================================================================
    IF ( Input_Opt%AnalyticalInv  .or. Input_Opt%UseEmisSF .or. &
         Input_Opt%UseWetlandSF   .or. Input_Opt%UseOHSF ) THEN
       ...
       DO I = 1, State_Grid%NX
          ...
          IF ( Input_Opt%UseWetlandSF ) THEN
             ! Remove wetland emissions from total
             CH4_EMIS(I,J,1)  = CH4_EMIS(I,J,1) - CH4_EMIS(I,J,11)

             ! Scale wetland emissions separately
             CH4_EMIS(I,J,11) = CH4_EMIS(I,J,11) * WETLAND_SF(I,J)
          ENDIF

          ! Scale total emissions (minus wetland)
          IF ( Input_Opt%UseEmisSF ) THEN
             CH4_EMIS(I,J,1) = CH4_EMIS(I,J,1) * EMIS_SF(I,J)
          ENDIF

          IF ( Input_Opt%UseWetlandSF ) THEN
             ! Add wetland emissions back to total
             CH4_EMIS(I,J,1)  = CH4_EMIS(I,J,1) + CH4_EMIS(I,J,11)
          ENDIF
          ...
          IF ( Input_Opt%AnalyticalInv ) THEN
             ...
             IF ( Input_Opt%ClusterNumber .GT. 0 ) THEN
                IF ( CLUSTERS(I,J) == Input_Opt%ClusterNumber ) THEN
                   CH4_EMIS(I,J,1) = CH4_EMIS(I,J,1) * Input_Opt%PerturbEmis
                   !Print*, 'Analytical Inversion: Scaled cluster ', &
                   !        Input_Opt%ClusterNumber, ' by ', &
...
!
    ...
    IF ( Input_Opt%UseOHSF ) THEN
       ...
       CALL HCO_EvalFld( HcoState, 'OH_SF', OH_SF, RC)
       IF ( RC /= GC_SUCCESS ) THEN
